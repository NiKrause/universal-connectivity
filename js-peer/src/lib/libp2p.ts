...
function getConfiguredRelayBootstrapAddrs(): string[] {
...
export async function startLibp2p(): Promise<Libp2pType> {
  ...
  libp2p = await createLibp2p({
    ...
    transports: [
      ...
      webSockets(),
      webRTC(),
      // 👇 Required to establish connections with peers supporting WebRTC-direct, e.g. the Rust-peer
      webRTCDirect(),
    ...
    services: {
      ...
      // Delegated routing helps us discover the advertised multiaddrs of the dedicated bootstrap peers
...
// messages could be perceived as duplicate if this isn't added
...
async function getRelayBootstrapAddrs(client: DelegatedRoutingV1HttpApiClient): Promise<string[]> {
  const configuredRelayBootstrapAddrs = getConfiguredRelayBootstrapAddrs()
  if (configuredRelayBootstrapAddrs.length > 0) {
    log('using NEXT_PUBLIC_RELAY_LISTEN_ADDRS override as explicit relay bootstrap addresses')
    return configuredRelayBootstrapAddrs
  }
